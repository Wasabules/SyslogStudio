...
func (a *App) PreviewLogFile(path string) (importer.Preview, error) {
	if path == "" {
		return importer.Preview{}, fmt.Errorf("no file chosen")
	}
	return importer.PreviewFile(a.importOptions(path))
}
...
func (a *App) ImportLogFile(path string, persist bool) (importer.Result, error) {
	if path == "" {
		return importer.Result{}, fmt.Errorf("no file chosen")
	}

	opts := a.importOptions(path)
...
func (a *App) importOptions(path string) importer.Options {
